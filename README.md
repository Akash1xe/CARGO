# CargoFlow — Distributed Intercity Logistics and Cargo-Capacity Booking Platform
...
CargoFlow demonstrates a complete microservices architecture for distributed logistics and cargo-capacity booking, covering:
...
|---|---|---|
| PostgreSQL 15 | 5432 | `admin` / `cargoflow_dev_password` |
| pgAdmin | 8081 | http://localhost:8081 — `admin@admin.com` / `admin` |
| Redis Stack | 6379 (Redis), 8001 (RedisInsight) | password `cargoflow_dev_password` — RedisInsight at http://localhost:8001 |
| Zookeeper | 2181 | — |
...
```bash
git clone https://github.com/YOUR_USERNAME/cargoflow.git
cd cargoflow
```
...
Access tokens contain the trusted user ID and `CUSTOMER` or `ADMIN` role. CargoFlow Admin APIs require an `ADMIN` access token at both the API Gateway and Admin Service boundaries.

To bootstrap a development Admin, set `CARGOFLOW_ADMIN_EMAIL`, `CARGOFLOW_ADMIN_PASSWORD`, `CARGOFLOW_ADMIN_FIRST_NAME`, and `CARGOFLOW_ADMIN_LAST_NAME` together with `DATABASE_URL`, then run:
...
The seed is environment-driven, hashes the password, and never contains or prints the password.

**Database:** PostgreSQL `cargoflow_user` (models: `User`, `AuthProvider`)
...
**Database:** PostgreSQL `cargoflow_admin`
...
**Database:** PostgreSQL `cargoflow_booking` (`Booking` with saga tracking, segment bookings)
...
**Database:** PostgreSQL `cargoflow_payment` (models: `PaymentOrder`, `Refund`, `PaymentAuditLog`)
...
**Database:** PostgreSQL `cargoflow_inventory` (models: `ScheduleInventory`, `SeatInventory`, `RouteStop`, `SeatSegmentLock`, `IdempotencyRecord`)
...
REDIS_URL=redis://:cargoflow_dev_password@localhost:6379
...
DATABASE_URL=postgres://admin:cargoflow_dev_password@localhost:5432/cargoflow_user
REDIS_URL=redis://:cargoflow_dev_password@localhost:6379
...
DATABASE_URL=postgres://admin:cargoflow_dev_password@localhost:5432/cargoflow_admin
...
DATABASE_URL=postgres://admin:cargoflow_dev_password@localhost:5432/cargoflow_booking
REDIS_URL=redis://:cargoflow_dev_password@localhost:6379
...
DATABASE_URL=postgres://admin:cargoflow_dev_password@localhost:5432/cargoflow_payment
ALLOWED_ORIGINS=http://localhost:3000,http://localhost:4000
...
DATABASE_URL=postgres://admin:cargoflow_dev_password@localhost:5432/cargoflow_inventory
ALLOWED_ORIGINS=http://localhost:3000,http://localhost:4000
