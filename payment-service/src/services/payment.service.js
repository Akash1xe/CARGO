...
const createPaymentOrder = async (shipmentBookingId, amount, userId, idempotencyKey) => {
     if (!shipmentBookingId || !amount || !userId || !idempotencyKey) {
          throw new BadRequestError('shipmentBookingId, amount, userId, and idempotencyKey are required');
     }
     ...
     return withIdempotency(`payment-order:${idempotencyKey}`, async () => {
          ...
          const gatewayResult = await gateway.createOrder(amount, 'INR', shipmentBookingId, {
               shipmentBookingId,
               userId,
          });
          ...
          await prisma.paymentAuditLog.create({
               data: {
                    ...
                    metadata: { shipmentBookingId, userId, amount },
               },
          });
          ...
          return {
               paymentOrderId: paymentOrder.id,
               shipmentBookingId: paymentOrder.shipmentBookingId,
               gatewayOrderId: gatewayResult.gatewayOrderId,
...
const handlePaymentCaptured = async (paymentOrder, gatewayPaymentId, paymentEntity) => {
     ...
     await paymentProducer.publishPaymentSuccess(
          paymentOrder.id,
          paymentOrder.shipmentBookingId,
          gatewayPaymentId,
          paymentOrder.amount
     ).catch(err => {
...
const initiateRefund = async (paymentOrderId, amount, reason, idempotencyKey) => {
     ...
     return withIdempotency(`refund:${idempotencyKey}`, async () => {
          ...
          const gatewayResult = await gateway.initiateRefund(
               ...
               amount,
               { reason, shipmentBookingId: paymentOrder.shipmentBookingId }
          );
          ...
          const refund = await prisma.refund.create({
               data: {
                    ...
                    metadata: { shipmentBookingId: paymentOrder.shipmentBookingId, reason: reason || null },
               },
          });
